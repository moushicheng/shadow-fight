...
| 2.5 ✅  | 路线选择系统    | `core/run/RouteSelector.ts` — 精英/普通 A/普通 B 三选一 + 赏金挑战；生命机制（5 颗心） | 2.3      |
...
- 所有怪物由开发者在 MonsterTemplate 中精心设计（卡组、属性比例、遗物），不做随机拼凑
- 属性缩放：模板属性按层级等比缩放，普通 = 25 + 层数 × 3，精英 = floor+2 等效层，赏金 = 精英 × 1.5
- 模板过滤遵守 floorMin/floorMax 和类型限制（NORMAL 全层、FACTION 3+、ELITE 6+）
- 路线选择时从合法模板池中随机选取，支持排除去重
