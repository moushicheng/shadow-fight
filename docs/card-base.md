...
诅咒卡是特殊卡牌类型，来源有两种：咒术师卡牌在战斗中塞入对手卡组，或负面事件将诅咒卡加入玩家卡组。
...
1. 从卡组克隆中抽出所有诅咒卡（cardType === CURSE）
2. 逐张随机插入卡组的任意位置（使用战斗种子随机，保证可回放）

**规则说明**：

| 规则 | 说明 |
| --- | --- |
| 卡组管理中的位置 | 仅供参考，实际战斗位置随机 |
| 不可手动调序 | 商店"调整卡序"服务无法选中诅咒卡（UI 显示锁定图标） |
| 可以移除 | 商店"移除卡牌"服务可以删除诅咒卡（唯一的摆脱手段） |
| 战斗中塞入的诅咒 | 由咒术师卡牌在战斗中插入，遵循 insertPosition 规则（见 §三 3.2），战斗结束后自动丢弃 |

> **设计意图**：诅咒卡随机洗入让玩家无法通过卡序编排规避诅咒，诅咒在战斗中的任何时刻都可能打断出牌节奏。唯一的解法是花费金币在商店移除——这与"服务递增定价"机制配合，让诅咒卡成为持续的经济压力。
